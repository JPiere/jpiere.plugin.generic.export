 ...
 *
 */
public class JPiereGenericExportModelFactory implements IModelFactory {

	private static CCache<String,Class<?>> s_classCache = new CCache<String,Class<?>>(null, "PO_Class", 20, false);
	private final static CLogger s_log = CLogger.getCLogger(JPiereGenericExportModelFactory.class);

	@Override
	public Class<?> getClass(String tableName)
	{
		if(tableName.startsWith("JP_GenericExpFormat"))
		{
			...
			//	Search packages
			StringBuffer name = new StringBuffer("jpiere.plugin.generic.export.model").append(".M").append(className);
			...
			//	Adempiere Extension
			clazz = getPOclass("jpiere.plugin.generic.export.model.X_" + tableName, tableName);
			if (clazz != null)
			{
				s_classCache.put(tableName, clazz);
				return clazz;
